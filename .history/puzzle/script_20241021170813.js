...
function checkCompletion() {
    ...
    let completed = Array.from(tiles).every((tile, index) => {
        // Ensure that the empty tile is ignored in the completion check
        if (tile.src.includes("9.jpg")) {
            return true; // If the tile is the blank tile, consider it as correct
        }
        return tile.src === correctOrder[index];
    });
