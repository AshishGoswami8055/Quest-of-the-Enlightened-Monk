...
var maxTurns = 22; // Maximum number of turns allowed
...
function loadPuzzle(imagePaths) {
    ...
    document.getElementById("message").innerText = ""; // Clear any game over or success messages
...
function dragStart() {
    if (gameCompleted || turns >= maxTurns) return; // Prevent dragging if game is completed or turns exceed maxTurns
...
function dragDrop() {
    if (gameCompleted || turns >= maxTurns) return; // Prevent dropping if game is completed or turns exceed maxTurns
...
function dragEnd() {
    if (!otherTile || gameCompleted || turns >= maxTurns) return; // Prevent actions if no tile is selected, game is completed, or turns exceed maxTurns
...
function checkCompletion() {
    ...
    if (completed) {
        if (currentPuzzle === 1) {
            ...
            setTimeout(function() {
                currentPuzzle = 2; 
                loadPuzzle(imagePaths2); // Load the second puzzle after delay
            ...
            setTimeout(function() {
                currentPuzzle = 3; 
                loadPuzzle(imagePaths3); // Load the third puzzle after delay
                document.getElementById("message").innerText = ""; // Clear the message
            }, 3000); // Display message for 3 seconds
    
        } else if (currentPuzzle === 3) {
            console.log("Congratulations for completing all levels!");
            document.getElementById("message").innerText = "Congratulations for completing all levels! The last answer is logo.";
            
            setTimeout(function() {
                window.close(); // Close the game window after a short delay
            }, 3000); // 3-second delay before closing the window
        }
    }    
...
(function (d, w) {
    ...
    let board = Array(9).fill(null); // Tic-Tac-Toe board as a 3x3 grid
    const cells = d.querySelectorAll('.cell'); // Select all the cells
    const tictactoeContainer = d.querySelector('.tictactoe-container');
    const overlay = d.querySelector('.overlay');
    const startGame = d.querySelector('#startGame');
    const closeGame = d.querySelector('#closeGame');
    ...
    const handleCellClick = (e) => {
      ...
      // If cell is already filled or game is over, exit
      if (board[cellIndex]) return;
  
      // Mark cell with the current player (X or O)
    ...
    const resetGame = () => {
      board = Array(9).fill(null); // Clear the board array
      cells.forEach(cell => cell.textContent = ''); // Clear all cell content
      currentPlayer = "X"; // Reset to player X
    };
  
    // Function to show the Tic-Tac-Toe popup
    const showGame = () => {
      tictactoeContainer.style.display = 'block';
      overlay.style.display = 'block';
    };
  
    // Function to close the Tic-Tac-Toe popup
    const closeGamePopup = () => {
      tictactoeContainer.style.display = 'none';
      overlay.style.display = 'none';
    };
  
    // Event listener to start the game when sunflower image is clicked
    startGame.addEventListener('click', showGame);
  
    // Event listener to close the game popup
    closeGame.addEventListener('click', closeGamePopup);
  
    // Add click event listener for each Tic-Tac-Toe cell
    cells.forEach(cell => cell.addEventListener('click', handleCellClick));
  })(document, window);
  

// Function to handle game over when turns exceed 22
function gameOver() {
    console.log("Game Over! You've reached the maximum turns.");
    document.getElementById("message").innerText = "Game Over! You've reached the maximum turns.";

    // Automatically restart the game after 2 seconds (2000 milliseconds)
    setTimeout(function () {
        loadPuzzle(currentPuzzle === 1 ? imagePaths1 : currentPuzzle === 2 ? imagePaths2 : imagePaths3); // Reload the current puzzle
    }, 2000);
}
