...
function checkCompletion() {
    ...
    let completed = currentOrder.every((tileSrc, index) => {
        const tileFilename = tileSrc.split('/').pop(); // Extract the filename from the full path
        const isMatch = tileFilename === correctOrder[index]; // Compare the filenames
        console.log(`Comparing ${tileFilename} with ${correctOrder[index]}: ${isMatch}`);
        return isMatch;
    });    
