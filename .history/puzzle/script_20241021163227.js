...
// Include all image paths, including the 9th image
...
function createBoard() {
    ...
    for (let r = 0; r < rows; r++) {
        for (let c = 0; c < columns; c++) {
            ...
            // If the tile is empty, set a specific class to hide it
            if (tile.src === "") {
...
function dragEnd() {
    if (otherTile.style.visibility === "hidden") { // Check if the other tile is the empty tile
        ...
        if (isAdjacent) {
            ...
            if (otherImg === "") {
                otherTile.style.visibility = "hidden"; // Hide the empty tile after swapping
            }
