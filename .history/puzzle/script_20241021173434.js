...
function checkCompletion() {
    ...
    let currentOrder = Array.from(tiles).map(tile => tile.src.split('/').pop()); // Get current order of tiles

    // Log the current order of tiles to the console
    console.log("Current tile order: ");
    console.log(`(${currentOrder.length}) [${currentOrder.map(tile => `"${tile}"`).join(', ')}]`);
    ...
    let completed = currentOrder.every((tileSrc, index) => {
        return tileSrc === correctOrder[index]; // Compare only the filenames
    });
