...
function handleCollision() {
    ...
    if (
        checkCollision(player, ghostObj) && // Check if the player collides with the ghost
        invincibility == false && // Ensure the player is not invincible
        currentTime - lastGhostDamageTime >= ghostDamageCooldown // Ensure cooldown has passed
    ) {
        // health -= 5; // (Commented out) Old health deduction logic
        health -= characterType === "Warrior Monk" ? 3 : 5; // Deduct health based on character type
        updateHealth(); // Update health display
        ghostAudio.play(); // Play ghost sound upon collision
        lastGhostDamageTime = currentTime; // Update the last damage time
        if (health <= 0) { // Check if health has dropped to 0 or below
            console.log("Game Over"); // Log game over message
        }
    }

    const energies = document.querySelectorAll(".energy"); // Select all energy elements
    energies.forEach((energy) => { // Loop through each energy element
        // Check for collision with energy and if it has not faded out
        if (
            checkCollision(player, energy.querySelector("img")) &&
            !energy.classList.contains("fade-out")
        ) {
            golden_Energy++; // Increase golden energy count
            updateGoldenEnergy(); // Update the display of golden energy
    ...
    const harmfulObjects = document.querySelectorAll( // Select all harmful objects (wolves, bears, spikes, chopped trees)
        ".wolf, .bear, .spike, .chopped-tree"
    );
    harmfulObjects.forEach((obj) => { // Loop through each harmful object
        // Check for collision with player and if the object has not faded out
        if (
            checkCollision(player, obj.querySelector("img")) &&
            !obj.classList.contains("fade-out") &&
            invincibility == false // Ensure the player is not invincible
        ) {
            health -= 10; // Deduct health by 10
            updateHealth(); // Update health display
