...
function createPuzzle() {
    ...
    imagePaths.forEach((src, index) => {
        ...
        img.onload = () => {
            ...
            canvas.width = 100;  // Set the width for each piece
            canvas.height = 100; // Set the height for each piece
            const context = canvas.getContext('2d');

            context.drawImage(img, 0, 0, canvas.width, canvas.height);
            pieces[index] = canvas; // Store the canvas for shuffling
            puzzleContainer.appendChild(canvas);

            // Add click event to handle piece placement
            canvas.addEventListener('click', () => {
                // Handle piece placement logic here
                console.log(`Piece ${index + 1} clicked`); // Debugging log
            });
        };
    });
}
...
function shufflePuzzle() {
  ...
  pieces.forEach(piece => puzzleContainer.appendChild(piece)); // Re-add shuffled pieces
}

document.getElementById('shuffle-puzzle').addEventListener('click', shufflePuzzle);

document.addEventListener('DOMContentLoaded', () => {
  const player = document.getElementById('player'); // Replace with your actual player element
  ...
  setInterval(() => {
      ...
      // Check for collision with the puzzle
      if (checkCollision(playerRect, puzzleRect)) {
          console.log('Collision detected with puzzle!'); // Debugging log
          // Logic to start the puzzle game
          startPuzzleGame();
      }
  ...
  function startPuzzleGame() {
      // Your logic to initiate the puzzle game here
      console.log('Starting puzzle game...');
      // Example: Show puzzle grid or change the game state
      // You might want to display a new UI or load the puzzle images
  }
});
