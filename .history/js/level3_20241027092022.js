...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Start the animal flipping animation
  flipAnimal();

  // Function to update the displayed score
  function updateScore() {
    scoreElement.textContent = `Score : ${score}`; // Update score text on the UI
  }

  // Function to update the golden energy display and notify the player
  function updateGoldenEnergy() {
    // Check if enough golden energy is available to use the shield
    if (golden_Energy >= 5) {
      notification.innerHTML =
        "Now You can use the ENERGY Shield by pressing 'e'"; // Notify player
      setInterval(() => {
        notification.innerHTML = ""; // Clear notification after 5 seconds
      }, 5000);
    } else {
      notification.innerHTML = ""; // Clear notification if energy is insufficient
    }
    goldenElement.textContent = `${golden_Energy}`; // Update golden energy display
  }

  let sandstormDamageInterval; // Variable to hold the interval for sandstorm damage

  // Function to check proximity to the sandstorm and apply damage if necessary
  function checkSandStormProximity() {
    // Get the bounding rectangles of the player and sandstorm
    ...
    // Calculate the distance between the centers of the player and sandstorm
    ...
    // Check if the player is within the damaging range of the sandstorm
    if (distance <= 800) {
      // If the player is in range, start applying damage at regular intervals
      if (!sandstormDamageInterval) {
        sandstormDamageInterval = setInterval(() => {
          ...
          if (health <= 0) {
            clearInterval(sandstormDamageInterval); // Stop damage if health is zero
            // Handle game over scenario (e.g., call a function)
          }
        }, 1000); // Apply damage every second
      }
    } else {
      clearInterval(sandstormDamageInterval); // Stop damage if out of range
      sandstormDamageInterval = null; // Reset interval variable
    }

    // Check if the player is within range of the ghost
    if (distance_ghost <= 300) {
      ghostAudio.play(); // Play ghost audio
      ghost.classList.add("active"); // Activate ghost visual
      // console.log("Ghost attack"); // Debugging message (optional)
    } else {
      ghost.classList.remove("active"); // Deactivate ghost visual
      // console.log("Ghost See it"); // Debugging message (optional)
    }
  }
