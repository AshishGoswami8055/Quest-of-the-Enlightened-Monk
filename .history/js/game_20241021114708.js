  ...
  function handleCollision() {
    ...
    hermitObj1.forEach((obj) => {
      ...
      meditationObjects.forEach(obj => {
        if (checkCollision(player, obj.querySelector('img')) && !obj.classList.contains('fade-out')) {
          if (fighting_Game == false) { // Assuming you want this interaction only when not in fighting mode
            isPlayable = false; // Disable gameplay
            fadeOutAndRemove(obj); // Fade out and remove meditation object

            // Add your meditation interaction logic here (e.g., open meditation page, play sound, etc.)
            // For example, you might want to open a meditation page:
            window.open("meditation.html"); // Replace with the actual meditation page URL
