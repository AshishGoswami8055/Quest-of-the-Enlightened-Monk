...
(function () {
  ...
  const sunflowers = document.querySelectorAll('.sunflower'); // Select sunflower elements
  ...
  const handleCellClick = (event) => {
    ...
    // Prevent further clicks if cell is already occupied or game is not active
  ...
  const checkResult = () => {
    ...
    if (roundWon) {
      displayTicTacToePopup(`Player ${currentPlayer} wins!`);
      isGameActive = false; // Prevent further moves
      return;
    }

    // Check for a draw
    if (!board.includes('')) {
      displayTicTacToePopup("It's a draw!");
    }
  ...
  // Function to display the Tic-Tac-Toe result in a popup
  const displayTicTacToePopup = (message) => {
    popupMessage.textContent = message;
    popup.style.display = 'block';
  };

  // Function to display a sunflower interaction popup
  const displaySunflowerPopup = (message) => {
    const sunflowerPopup = document.querySelector('.sunflower-popup');
    const overlay = document.querySelector('.overlay');

    // Update the message in the sunflower popup
    sunflowerPopup.textContent = message;

    // Display the popup and overlay
    sunflowerPopup.classList.add('show');
    overlay.classList.add('show');
  };

  // Add event listeners to sunflower elements
  sunflowers.forEach(sunflower => {
    sunflower.addEventListener('click', () => {
      displaySunflowerPopup("You found a sunflower!");
    });
  });
  ...
  restartButton1.addEventListener('click', () => {
    ...
    popup.style.display = 'none'; // Close any popups on restart
  });

  // Function to handle sunflower interactions
  const handleSunflowerClick = (event) => {
    displaySunflowerPopup("You found a sunflower!");
  };

  // Add event listeners to Tic-Tac-Toe cells
  cells.forEach(cell => {
  ...
  // Add event listeners to sunflower elements
  sunflowers.forEach(sunflower => {
    sunflower.addEventListener('click', handleSunflowerClick);
  });

  // Function to handle Tic-Tac-Toe pop-up trigger
  const triggerTicTacToePopup = () => {
    // After memory game finishes, trigger the Tic-Tac-Toe pop-up
  ...
  // You can call this `triggerTicTacToePopup()` when your memory game is finished.
})();
