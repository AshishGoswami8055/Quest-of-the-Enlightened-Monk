...
function handleNextButtonClick() {
  ...
  // If there are more questions left in the quiz
  if (currentQuestionIndex < quizData.length) {
    saveProgress(); // Save the current progress before loading the next question
    loadQuestion(); // Load the next question
  } else {
    saveProgress(); // Save progress before displaying results when quiz is finished
    displayResults(); // Display the results after the quiz is completed
  }

  // Reset all radio buttons for the answers
...
// Check localStorage for saved progress when the DOM is fully loaded
retrieveProgress();

// memory game --------------------------------------------------------

// Immediately Invoked Function Expression (IIFE) to encapsulate the game logic
(function (d, w) {
  let cards = []; // Array to hold the card names
  let matches = 0; // Counter for matched pairs
  const flipped_cards = []; // Array to hold the currently flipped cards

  // Mapping of card names to their corresponding image file names
  const imageMapping = {
    react: "bear.png",
    vuejs: "blackrock.png",
    angular: "cloud.png",
  ...
  const build_board = () => {
    const board = d.querySelector(".board"); // Select the game board element
    cards = get_cards(); // Get the shuffled cards
    const card_items = cards
      .map((card, id) => {
        // Create card elements with unique data attributes
        return `<li class="card" data-id="${id}"></li>`;
      })
      .join(""); // Join all card items into a single string
  ...
  const flip_card = (e) => {
    const card = e.target.closest(".card"); // Get the clicked card element
    // Ensure valid card is clicked (not already matched or flipped)
    ...
    card.classList.add("is-flipped"); // Add class indicating the card is flipped

    // Store the flipped card's details
    flipped_cards.push({
      card,
      name: cards[card.dataset.id], // Card name
    });
  ...
  const check_match = () => {
    ...
    if (flipped_cards[0].name === flipped_cards[1].name) {
      flipped_cards.forEach((flipped_card) => {
        ...
        flipped_card.card.classList.remove("is-flipped"); // Remove the flipped class
      });

      matches += 2; // Increment the match counter
      flipped_cards.length = 0; // Clear the flipped cards array

      // Check if the game is over (all matches found)
      if (game_over()) {
        console.log("You Won now move to next part!!!", isPlayable); // Log victory
      ...
      w.setTimeout(() => {
        flipped_cards.forEach((flipped_card) => {
          const card = flipped_card.card; // Reference to the flipped card
          card.style.backgroundImage = ""; // Clear the card's image
  ...
  const shuffle_cards = (stack) => {
    ...
    let i = 0; // Counter for shuffled cards

    // Loop to select random cards without duplication
    while (i < total) {
      const number = Math.floor(Math.random() * total); // Generate random index
      if (!random_numbers.includes(number)) { // Ensure index has not been used
        shuffled.push(stack[number]); // Add card to shuffled array
        random_numbers.push(number); // Track used index
        i++; // Increment counter
      }
    }

    return shuffled; // Return shuffled array of cards
  };

  // Function to check if the game is over (all matches found)
  const game_over = () => {
    if (matches === cards.length) { // If matches equal total cards
      matches = 0; // Reset matches for potential next round
      return true; // Game is over
    }
    return false; // Game is not over
  };

  // Function to get and shuffle the card stack
  const get_cards = () => {
    const stack = ["react", "vuejs", "angular", "css3", "html5", "js"]; // Initial card set

    // Duplicate the stack for pairs (two of each card)
    const full_stack = stack.concat(stack);

    return shuffle_cards(full_stack); // Shuffle and return the full stack
  };

  build_board(); // Call the function to build the game board
})(document, window); // Pass document and window objects to the IIFE


// memory game --------------------------------------------------------

(function (d, w) {
  // Initialize variables to keep track of cards, matches, and flipped cards
  let cards = [];
  let matches = 0;
  const flipped_cards = [];
  ...
  // Function to build the game board
  const build_board = () => {
    const board = d.querySelector(".board1"); // Select the board element
    cards = get_cards(); // Retrieve the shuffled card array
    // Create HTML for each card and set it in the board
    const card_items = cards
      .map((card, id) => {
        return `<li class="card" data-id="${id}"></li>`; // Each card has a unique data-id
      })
      .join(""); // Join all card items into a single string

    board.innerHTML = card_items; // Add cards to the board
  ...
  const flip_card = (e) => {
    ...
    if (flipped_cards.length == 2) return; // Only allow flipping two cards at a time
  ...
  const check_match = () => {
    // If both flipped cards have the same name
    if (flipped_cards[0].name === flipped_cards[1].name) {
      flipped_cards.forEach((flipped_card) => {
        flipped_card.card.classList.add("is-match"); // Mark as a match
        flipped_card.card.classList.remove("is-flipped"); // Remove flipped state
      });

      matches += 2; // Increase match count
      flipped_cards.length = 0; // Clear flipped cards

      // Check if the game is over
      if (game_over()) {
        console.log("You Won! Now move to the next part!!!");
        $(".container1").addClass("hidden"); // Hide the game container
      ...
      w.setTimeout(() => {
        flipped_cards.forEach((flipped_card) => {
          ...
          card.classList.remove("is-flipped"); // Remove flipped state
        });
        flipped_cards.length = 0; // Clear flipped cards
      }, 800);
    }
  ...
  const shuffle_cards = (stack) => {
    ...
    while (i < total) {
      ...
      if (!random_numbers.includes(number)) { // Ensure it's a unique card
        shuffled.push(stack[number]); // Add to shuffled cards
        random_numbers.push(number); // Record the number
        i++; // Move to next card
      }
    }

    return shuffled; // Return shuffled cards
  };
  ...
  const get_cards = () => {
    ...
    return shuffle_cards(full_stack); // Return shuffled pairs
  };
