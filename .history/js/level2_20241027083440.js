...
(function (d, w) {
  // Array to store card names and track matches count
  let cards = [];
  let matches = 0;
  const flipped_cards = []; // Array to store currently flipped cards for matching check

  // Object mapping card names to image filenames
  const imageMapping = {
    react: "bear.png",
    vuejs: "blackrock.png",
    angular: "cloud.png",
  ...
  // Function to build the game board by creating card elements
  const build_board = () => {
    const board = d.querySelector(".board1"); // Select board container
    ...
    board.innerHTML = card_items; // Add card HTML to the board
    board.addEventListener("click", flip_card); // Set click event listener for card flipping
  };

  // Function to handle card flip events
  const flip_card = (e) => {
    const card = e.target.closest(".card"); // Identify clicked card
    if (!card || card.matches(".is-match") || card.matches(".is-flipped"))
    ...
    card.style.backgroundImage = `url(images/${
      imageMapping[cards[card.dataset.id]]
    })`; // Display card image based on name
    card.classList.add("is-flipped"); // Mark card as flipped

    flipped_cards.push({
      card,
      name: cards[card.dataset.id],
    }); // Store flipped card information for matching

    if (flipped_cards.length == 2) {
      check_match(); // Check for a match when two cards are flipped
    }
  };
  ...
  const check_match = () => {
    if (flipped_cards[0].name === flipped_cards[1].name) {
      ...
      flipped_cards.forEach((flipped_card) => {
        ...
        flipped_card.card.classList.remove("is-flipped"); // Remove flipped status
      });

      matches += 2; // Increment match count
      flipped_cards.length = 0; // Reset flipped cards array

      if (game_over()) {
        // Check if all matches are found (game over)
        console.log("You Won now move to next part!!!", isPlayable);
        $(".container1").addClass("hidden"); // Hide game container on win
        document.querySelector(".container1").style.visibility = "hidden";
      }
    } else {
      // If cards do not match, reset them after a delay
      w.setTimeout(() => {
        flipped_cards.forEach((flipped_card) => {
          const card = flipped_card.card;
          card.style.backgroundImage = ""; // Remove background image
          card.classList.remove("is-flipped"); // Unflip card
        });
        flipped_cards.length = 0; // Clear flipped cards array
      }, 800);
    }
  };
  ...
  const shuffle_cards = (stack) => {
    const shuffled = []; // Array to store shuffled cards
    const random_numbers = []; // Array to track random indices
    const total = stack.length;
    let i = 0;

    while (i < total) {
      // Randomly select cards until all are shuffled
    ...
    return shuffled; // Return shuffled card array
  };

  // Function to check if the game is over (all cards matched)
  const game_over = () => {
    if (matches === cards.length) {
      matches = 0; // Reset match counter for potential new game
      return true; // Game is over
    }
    return false; // Game is still ongoing
  };

  // Function to create a stack of card names and duplicate for matching pairs
  const get_cards = () => {
    ...
    return shuffle_cards(full_stack); // Shuffle and return cards
  };

  build_board(); // Initialize game board
})(document, window);


//memory game final
(function (d, w) {
  // Variables for cards array, matches counter, and array for flipped cards
  let cards = [];
  let matches = 0;
  const flipped_cards = [];
  ...
  // Function to build the board by creating card elements and adding them to the DOM
  const build_board = () => {
    ...
    board.addEventListener("click", flip_card); // Set event listener for flipping cards
  };
  ...
  const flip_card = (e) => {
    ...
    if (flipped_cards.length == 2) return; // Limit flipping to two cards at a time

    card.style.backgroundImage = `url(images/${
      imageMapping[cards[card.dataset.id]]
    })`; // Set background image based on card name
    card.classList.add("is-flipped"); // Mark card as flipped
    ...
    if (flipped_cards.length == 2) {
      check_match(); // Check for a match if two cards are flipped
    }
  };
  ...
  const check_match = () => {
    if (flipped_cards[0].name === flipped_cards[1].name) {
      ...
      if (game_over()) {
        console.log("You Won now move to next part!!!", isPlayable); // Notify user of win
        $(".container2").addClass("hidden"); // Hide game container on win
      ...
      w.setTimeout(() => {
        flipped_cards.forEach((flipped_card) => {
          const card = flipped_card.card;
          card.style.backgroundImage = ""; // Reset card background
          card.classList.remove("is-flipped"); // Unflip card
        });
        flipped_cards.length = 0; // Clear flipped cards array
      }, 800);
  ...
  // Function to generate a stack of cards and duplicate them for pairs
  const get_cards = () => {
