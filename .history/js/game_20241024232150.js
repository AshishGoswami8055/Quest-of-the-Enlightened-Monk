...
var mark;
var cells;
var isPlayable = true; // Control whether gameplay is active

// Function to set the player based on radio button selection
function setPlayer() {
    mark = this.value;
...
// Function to switch the player mark
function switchMark() {
    mark = (mark === 'X') ? 'O' : 'X';
}
...
// Function to build the game grid
function buildGrid() {
    for (var i = 1; i <= 9; i++) {
...
// Collision detection logic for the sunflower
const sunflowers = document.querySelectorAll(".sunflower");
const player = {}; // Define your player object according to your game logic

sunflowers.forEach((sunflower) => {
    if (checkCollision(player, sunflower) && isPlayable) {
        document.querySelector(".ticTacToeContainer").style.visibility = "visible"; 
        console.log("Collision with sunflower detected");
        isPlayable = false; // Disable gameplay when Tic Tac Toe is active
    }
});

// Example collision checking function
function checkCollision(player, sunflower) {
    // Implement your collision detection logic here
    // Return true if a collision is detected
    return false; // Placeholder for actual collision detection
}
