...
document.addEventListener("DOMContentLoaded", () => {
  ...
  */
  ...
  function handleCollision() {
    ...
    // Check collision with meditation objects 24
    const meditationObj = document.querySelectorAll(".meditation");
    meditationObj.forEach((obj) => {
      if (
        checkCollision(player, obj.querySelector("img")) &&  // Check if the player collides with the meditation image
        !obj.classList.contains("fade-out")                  // Ensure the meditation object is not fading out
      ) {
        // Optional: health restoration or special meditation logic
        // health = 100;
        // updateHealth();
        // isPlayable = false; // Disable gameplay if necessary

        document.querySelector(".meditation-container").style.visibility = "visible"; // Show meditation popup on collision
      }
    });
