...
document.addEventListener("DOMContentLoaded", () => {
  ...
  meditationObj.forEach((obj) => {
    if (
      checkCollision(player, obj.querySelector("img")) && // Check if player collides with meditation image
      !obj.classList.contains("fade-out") // Ensure the object is not fading out
    ) {
      // Uncomment and implement health restoration or meditation logic as needed
      // health = 100; 
      // updateHealth();
      // isPlayable = false; // Disable gameplay

      // Make meditation popup visible
...
meditationButton.addEventListener('click', function() {
  ...
  const audio = new Audio('images/meditation.mp3');
