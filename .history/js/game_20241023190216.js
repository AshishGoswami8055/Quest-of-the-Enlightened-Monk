 ...
 sunflowers.forEach((sunflower) => {
   if (checkCollision(player, sunflower)) {
    console.log("Collision with sunflower detected");
     // Handle any interactions with sunflowers, but allow movement
     // Example: Reduce health if necessary but do not block movement
     if (invincibility === false) {
       health -= 2; // Example health deduction
       updateHealth();
     }
   }
 });
...
(function() {
  ...
  // Function to show the sunflower interaction popup
  const handleSunflowerClick = (event) => {
      displayPopup("You found a sunflower!");
  };
  ...
  // Add event listeners to sunflowers
  sunflowers.forEach(sunflower => {
