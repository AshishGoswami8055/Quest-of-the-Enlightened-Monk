  ...
  */
  ...
  function handleCollision() {
    ...
    hermitObj1.forEach((obj) => {
      ...
      const meditation = document.querySelectorAll('.meditation');
      meditation.forEach(obj => {
          if (checkCollision(player, obj.querySelector('img')) && !obj.classList.contains('fade-out')) {
              if (someCondition == true) { // Use the appropriate condition
                  isPlayable = false; // Disable gameplay
                  fadeOutAndRemove(obj); // Fade out and remove meditation object
                  window.open("meditation/html/index.html"); // Open meditation game
                  setTimeout(function () {
                      isPlayable = true; // Re-enable gameplay after 3 seconds
                  }, 3000);
              }
