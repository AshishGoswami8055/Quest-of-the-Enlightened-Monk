...
const sunflowerObjects = document.querySelectorAll('.sunflower'); // Select all sunflower elements
sunflowerObjects.forEach(obj => {
    if (
        checkCollision(player, obj.querySelector("img")) && // Check if player collides with sunflower
        !obj.classList.contains("fade-out") // Ensure sunflower is not fading out
    ) {
        // Perform actions upon collision with sunflower
        health += 5; // Restore health or perform another action
        updateHealth(); // Update health display
        fadeOutAndRemove(obj); // Fade out and remove the sunflower element
        sunflowerAudio.play(); // Play sound effect for sunflower collision
    }
});
...
(function() {
  ...
  // Function to display the result in a popup
