...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Check for collision with meditation object
  ...
  meditationObj.forEach((obj) => {
    if (
      ...
      !obj.classList.contains("fade-out") // Ensure not fading out
    ) {
      document.querySelector(".meditation-container").style.visibility = "visible"; // Show container
  ...
  update();
    ...
    const meditationButton = document.getElementById('meditation');

    // Close button functionality
    document.getElementById("close-popup").addEventListener("click", function() {
      document.querySelector(".meditation-container").style.visibility = "hidden"; // Hide container
      const meditationVideo = document.getElementById("meditation-video");
      meditationVideo.pause(); // Pause the video when closing
...
meditationButton.addEventListener('click', function() {
  ...
  const audio = new Video('audio/meditation.mp4');
  console.log(video); // Check if audio object is created correctly
  audio.play().then(() => {
