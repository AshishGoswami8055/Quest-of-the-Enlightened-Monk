...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function handleCollision() {
    ...
    // Detect collision with meditation object and show the popup
    ...
    meditationObj.forEach((obj) => {
        if (checkCollision(player, obj.querySelector("img")) && !obj.classList.contains("fade-out")) {
            document.querySelector(".meditation-container").style.visibility = "visible";
            meditationVideo.play(); // Play video when popup is shown
        }
    });
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      console.log('Play button:', playButton); // Check if the play button is selected correctly
      console.log('Pause button:', pauseButton); // Check if the pause button is selected correctly
      console.log('Close button:', closeButton); // Check if the close button is selected correctly
  
      // Add event listeners only if buttons exist
      if (playButton) {
          playButton.addEventListener('click', () => {
              console.log('Play button clicked'); // Log when the play button is clicked
              if (video) {
                  video.play();
                  console.log('Video playing'); // Log when the video starts playing
              }
          });
      } else {
          console.warn('Play button not found'); // Warn if the play button is not found
      }
  
      if (pauseButton) {
          pauseButton.addEventListener('click', () => {
              console.log('Pause button clicked'); // Log when the pause button is clicked
              if (video) {
                  video.pause();
                  console.log('Video paused'); // Log when the video is paused
              }
          });
      } else {
          console.warn('Pause button not found'); // Warn if the pause button is not found
      }
      ...
      if (closeButton) {
          closeButton.addEventListener('click', () => {
              ...
              if (video) {
                  ...
                  console.log('Video reset to beginning'); // Log video reset
              }
              meditationContainer.style.visibility = 'hidden'; // Hide the popup
              document.getElementById('video-popup').classList.remove('active'); // Hide the popup
              console.log('Popup hidden'); // Log when the popup is hidden
          });
      } else {
          console.warn('Close button not found'); // Warn if the close button is not found
      }
  });
