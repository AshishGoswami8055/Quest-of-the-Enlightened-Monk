...
// Function to render the scoreboard displaying user answers
function renderScoreboard() {
  scoreboardBody.innerHTML = ""; // Clear previous scoreboard entries
  userAnswers.forEach((answer, index) => {
    const row = document.createElement("tr"); // Create a new row for each answer
    const questionCell = document.createElement("td"); // Cell for the question
    ...
    questionCell.textContent = `Q${index + 1}: ${answer.question}`; // Set question text in cell
    yourAnswerCell.textContent = answer.yourAnswer; // Set user's answer in cell
    correctAnswerCell.textContent = answer.correctAnswer; // Set correct answer in cell

    // Append cells to the row
    row.appendChild(questionCell);
    row.appendChild(yourAnswerCell);
    row.appendChild(correctAnswerCell);
    scoreboardBody.appendChild(row); // Append row to the scoreboard body
  });
}

// Function to restart the quiz and save the current game state
function restartQuiz() {
  ...
  // Redirect to the scoreboard page and close the current quiz window
  window.open('ScoreBoard.html');
  window.close('level3.html');
}

// Add event listeners for button clicks
nextButton.addEventListener("click", handleNextButtonClick); // Next button click
restartButton.addEventListener("click", restartQuiz); // Restart button click

// Check localStorage for saved progress when the DOM is loaded
retrieveProgress(); // Load any saved progress if available
