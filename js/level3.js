...
function startRainEffect() {
  const rivers = document.querySelectorAll(".river img"); // Select all river elements
  rivers.forEach(riverss => {
    riverss.classList.add("river-grow"); // Add grow class to trigger CSS animation
  });
...
function stopWeatherEffects() {
  const rivers = document.querySelectorAll(".river img"); // Select all river elements
  rivers.forEach(riverss => {
    riverss.classList.remove("river-grow"); // Add grow class to trigger CSS animation
  });
  rainSound.pause();
  thunderSound.pause();
...
let isNight = false; // Initially set to night
...
function startNightDayCycle() {
  const animals = document.querySelectorAll(".wolf img, .bear img");
  const snake = document.querySelectorAll(".snake");
  snake.forEach((snakes)=>{
    if(isNight){
      if (snakes.closest(".snake")) {
        snakes.classList.remove("snakeVisibility");
      } 
    }
    else{
      if (snakes.closest(".snake")) {
        snakes.classList.add("snakeVisibility");
      } 
    }
  })
  animals.forEach((animal) => {
    if (isNight) {
      if (animal.closest(".wolf")) {
        animal.classList.add("wolf-grow"); // Apply wolf-specific grow effect
      } else if (animal.closest(".bear")) {
        animal.classList.add("bear-grow"); // Apply bear-specific grow effect
      }
    } else {
      animal.classList.remove("wolf-grow", "bear-grow"); // Remove both grow effects during the day
    }
  });
  setInterval(() => {
    isNight = !isNight;
  ...
  }, 35000); // 35,000 ms = 35 seconds
}

function startCollisionChecks() {
  const harmfulObjects = document.querySelectorAll(".wolf, .bear, .spike, .chopped-tree");

  function checkHarmfulObjectCollisions() {
    harmfulObjects.forEach((obj) => {
      if (
        ...
        // Apply increased damage at night for wolves and bears
        if ((obj.classList.contains("wolf") || obj.classList.contains("bear")) && isNight) {
          damage = characterType === "Warrior Monk" ? 10 : 15; // Higher damage at night
          console.log("Increased damage at night:", damage);
        } else {
          damage = characterType === "Warrior Monk" ? 7 : 10; // Regular damage
          console.log("Regular damage:", damage);
        }

        health -= damage; // Apply calculated damage
  ...
  console.log(isNight ? "It's night now." : "It's day now.");
  // Set an interval to run the collision check frequently, e.g., every 100 ms
  setInterval(checkHarmfulObjectCollisions, 100);
}
    ...
    const snowMan15 = document.querySelectorAll('.snowman15');
    snowMan15.forEach(obj => {
      if (checkCollision(player, obj.querySelector('img')) && !obj.classList.contains('fade-out')) {
  ...
  const quizData = [
    {
        question: "What did Buddha tell Maya about the source of suffering?",
        options: ["It comes from wealth", "It comes from beauty", "It comes from attachment", "It comes from being misunderstood"],
        correct: "It comes from attachment"
    },
    {
        question: "In the story of Ananda, what does the withering leaf symbolize?",
        options: ["The beauty of nature", "The impermanence of life", "The strength of life", "The cycle of rebirth"],
        correct: "The impermanence of life"
    },
    {
        question: "According to Buddha’s advice, what is necessary to find true happiness?",
        options: ["Seeking material wealth", "Letting go of pain and desires", "Gaining more knowledge", "Attaching oneself to others"],
        correct: "Letting go of pain and desires"
    },
    {
        question: "What lesson did the woman searching for a mustard seed learn?",
        options: ["Happiness is a personal achievement", "Suffering is unique to her", "Suffering is universal and connects us all", "Joy is fleeting and temporary"],
        correct: "Suffering is universal and connects us all"
    },
    {
        question: "What advice did Buddha give Ananda to achieve peace?",
        options: ["Seek wisdom from others", "Embrace the present moment", "Avoid all forms of attachment", "Spend time in solitude"],
        correct: "Embrace the present moment"
    },
    {
        question: "What is the main theme of these stories involving Buddha?",
        options: ["Importance of meditation", "Pursuit of personal desires", "Compassion, understanding, and acceptance", "Gathering wealth for a better life"],
        correct: "Compassion, understanding, and acceptance"
    },
    {
        question: "What practice involves calming the mind and finding inner stillness?",
        options: ["Meditation", "Attachment", "Desire"],
        correct: "Meditation"
    },
    {
        question: "What concept reflects the idea of 'what you give, you get back' and encourages mindful actions?",
        options: ["Meditation", "Karma", "Wealth"],
        correct: "Karma"
    },
    {
        question: "What often creates pride and separation, but letting go of it brings freedom and unity?",
        options: ["Ego", "Compassion", "Happiness"],
        correct: "Ego"
    }
];
